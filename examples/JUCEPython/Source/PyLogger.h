...
public:
	PyLogger():ListBox("Logger"){
		PythonWrap::errIntercept.addLogListener(this);
		setModel(this);
		setRowHeight(12);
	}
	~PyLogger(){
		PythonWrap::errIntercept.removeLogListener(this);
	...
	int size = PythonWrap::errIntercept.entries.size();
	for(int i = 0 ; i < size ; i++){
		if(PythonWrap::errIntercept.entries[i].msg!="\n"){
			entries.add(PythonWrap::errIntercept.entries[i]);
		}
		}
