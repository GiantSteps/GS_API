		...
		for e in tracks:
			if midi.MetaEvent.is_event(e.statusmsg):
				if e.metacommand == midi.TimeSignatureEvent.metacommand :
					if foundTimeSignatureEvent and (pattern.timeSignature != [e.numerator, e.denominator]): gsiolog.error(pattern.name+" : multiple time signature found, not supported, result can be alterated") 
